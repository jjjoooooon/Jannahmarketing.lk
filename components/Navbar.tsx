...
// Define constants outside to prevent recreation on render
const MENU_ITEMS = [
  { name: 'Home', href: '/' },
  { name: 'Shop', href: '/shop' },
...
// Animation Variants defined outside for stability
            ...
            >
              <div className="relative">
                ...
                <motion.div
                  ...
                  className="absolute inset-0 bg-[#CCFF00] blur-lg opacity-20 rounded-full will-change-transform"
                />
                <img
                  src={logo}
                  alt="Sunstar Logo"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-[#050505] z-40 flex flex-col items-center justify-center h-[100dvh] w-full touch-none"
          >
            {/* Background Decorations (Optimized with transform-gpu) */}
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
              <div className="absolute top-1/4 -left-20 w-64 h-64 bg-[#CCFF00] rounded-full mix-blend-exclusion filter blur-[100px] opacity-20 animate-pulse transform-gpu" />
              <div className="absolute bottom-1/4 -right-20 w-64 h-64 bg-purple-500 rounded-full mix-blend-exclusion filter blur-[100px] opacity-20 animate-pulse transform-gpu" />
            </div>
                ...
                >
                  <Link
                    ...
                    className={`text-4xl md:text-5xl font-black transition-all duration-300 font-['Plus_Jakarta_Sans'] uppercase tracking-tight block py-2 ${location.pathname === item.href
                      ? 'text-[#CCFF00]'
                      : 'text-transparent [-webkit-text-stroke:1px_rgba(255,255,255,0.5)] hover:text-[#CCFF00] hover:[-webkit-text-stroke:0px]'
                      }`}
                  >
                    {item.name}
                  </Link>
