          ...
          <div className="hidden md:flex items-center space-x-12">
            {menuItems.map((item) => (
              <motion.a
                key={item.name}
                href={item.href}
                className="relative text-sm uppercase tracking-widest font-bold text-gray-400 hover:text-white transition-colors font-['Inter'] group"
                whileHover={{ y: -2 }}
              >
                {item.name}
                <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-[#CCFF00] transition-all duration-300 group-hover:w-full box-shadow-[0_0_10px_#CCFF00]" />
              </motion.a>
            ))}
          </div>
            ...
            >
              {menuItems.map((item) => (
                <motion.a
                  ...
                  className="text-4xl md:text-5xl font-black text-transparent stroke-text hover:text-[#CCFF00] hover:stroke-0 transition-all duration-300 font-['Plus_Jakarta_Sans'] uppercase tracking-tight text-center w-full py-2"
                  variants={{
                    open: { y: 0, opacity: 1 },
              ...
              >
                <button 
                  onClick={() => setIsOpen(false)}
                  className="px-10 py-4 rounded-full bg-[#CCFF00] text-black font-bold uppercase text-sm tracking-widest shadow-[0_0_30px_rgba(204,255,0,0.3)] hover:scale-105 transition-transform font-['Plus_Jakarta_Sans'] w-full max-w-xs"
                >
                  Shop All Flavors
                </button>
              </motion.div>
