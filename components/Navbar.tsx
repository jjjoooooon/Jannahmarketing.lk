      ...
      >
        <div className="container mx-auto px-6 flex justify-between items-center relative z-50">
          <Link to="/" aria-label="Home" className="z-50 block transition-transform active:scale-95 group">
            <div className="flex items-center space-x-2">
              <div className="relative">
                ...
                <img src={logo} alt="Sunstar Logo" className="w-8 h-8 md:w-10 md:h-10 relative z-10 object-contain" loading="eager" width="40" height="40" />
              </div>
              <span className="text-xl md:text-2xl font-black tracking-tighter text-white font-display">SUNSTAR</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-4">
            <button onClick={() => setIsDrawerOpen(true)} className="relative p-2 text-white hover:text-brand-lime transition-colors group">
              <ShoppingCart size={24} />
              {totalItems > 0 && <span className="absolute -top-1 -right-1 bg-brand-lime text-black text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center border-2 border-brand-black group-hover:scale-110 transition-transform">{totalItems}</span>}
            </button>
            <Link to="/contact">
              <button className="flex items-center gap-2 px-6 py-2.5 rounded-full bg-brand-lime text-black font-bold text-xs uppercase tracking-widest font-display hover:bg-white hover:shadow-[0_0_20px_rgba(204,255,0,0.4)] active:scale-95 transition-all">
          ...
          <div className="flex md:hidden items-center gap-4">
            <button onClick={() => setIsDrawerOpen(true)} className="relative p-2 text-white active:text-brand-lime transition-colors" aria-label="Open Cart">
              <ShoppingCart size={24} />
              {totalItems > 0 && <span className="absolute -top-1 -right-1 bg-brand-lime text-black text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center border-2 border-brand-black">{totalItems}</span>}
            </button>
            <button className="text-white z-50 p-2 focus:outline-none relative touch-manipulation" onClick={() => setIsOpen(!isOpen)} aria-label={isOpen ? "Close Menu" : "Open Menu"}>
